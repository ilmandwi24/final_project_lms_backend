...
const getRecomendationAnime = async () => {
  try {
    const dataFromRedis = await Redis.getKey('recomendation');
    if (dataFromRedis) {
      CommonHelper.log(['Sample Helper', 'getRecomendationAnime', 'INFO'], {
        message: 'Get data from redis',
        key: 'recomendation'
      });
      return JSON.parse(dataFromRedis);
    }
    const list = await fetch('https://api.jikan.moe/v4/recommendations/anime');
    const data = await list.json();
    await Redis.setWithExpire('recomendation', JSON.stringify(data), 86400);

    return data;
  } catch (error) {
    CommonHelper.log(['Sample Helper', 'getRecomendationAnime', 'ERROR'], { message: `${error}` });
    throw CommonHelper.errorResponse(error);
  }
};
